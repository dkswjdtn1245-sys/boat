...
# 🌟 수질 데이터 변환 함수 (실측 보정 및 극한 오염 예외처리 완료)
def get_turbidity_ntu(raw_voltage):
    # [보정 1] 노이즈 억제 및 시연용 데드존 설정 (4.05V 이상은 무조건 맑은 물)
    if raw_voltage >= 4.05:
        return 0.0
    
    # [보정 2] 맑은 물 기준 4.10V를 공식의 영점(4.25V)에 맞추기 위한 오프셋(+0.15V) 추가
    comp_voltage = raw_voltage + 0.15
    
    # 🚨 [핵심 수정] 커피/흙물 등 극심한 오염 처리 🚨
    # 공식 꼭짓점인 2.56V(약 3000 NTU) 밑으로 전압이 떨어지면(예: 0.4V) 
    # 수학적으로 값이 감소(음수화)해버리므로, 센서 최대 한계치인 3000 NTU로 고정!
    if comp_voltage < 2.56:
        return 3000.0
    
    # 2차 방정식 변환식 적용 (2.56V ~ 4.2V 구간 정상 작동)
